...
//KDK Kick
void kdk_k(int p0, int p1, const double mass[], double pxyz[][2-0], double vxyz[][2-0]){
	double r12	= pow(pow(pxyz[p0][0]-pxyz[p1][0],2) + pow(pxyz[p0][1]-pxyz[p1][1],2), 0.5);
...
//KDK Drift
void kdk_d(int p0, double pxyz[][2-0], double vxyz[][2-0]){
...
//DKD Drift
void dkd_d(int p0, double pxyz[][2-0], double vxyz[][2-0]){
...
//DKD Kick
void dkd_k(int p0, int p1, const double mass[], double pxyz[][2-0], double vxyz[][2-0]){
        double r12      = pow(pow(pxyz[p0][0]-pxyz[p1][0],2) + pow(pxyz[p0][1]-pxyz[p1][1],2), 0.5);
        double force	= G*mass[p1]/(r12*r12);
        double ax       = force*( pxyz[p1][0] - pxyz[p0][0] )/r12;
        double ay       = force*( pxyz[p1][1] - pxyz[p0][1] )/r12;
...
//RK4
//k1
void rk4_k1(int p0, int p1, const double mass[], double pxyz[][2-0], double vxyz[][2-0], double k1_a[][2-0], double k1_v[][2-0], double buf_p2[][2-0]){
	k1_v[p0][0]	= vxyz[p0][0];
	k1_v[p0][1]	= vxyz[p0][1];
//	k1_v[p0][2]	= vxyz[p0][2];
	
	double kk1_r12		= pow(pow(pxyz[p0][0]-pxyz[p1][0],2) + pow(pxyz[p0][1]-pxyz[p1][1],2), 0.5);
...
void rk4_k2(int p0, int p1, const double mass[], double pxyz[][2-0], double vxyz[][2-0], double k2_a[][2-0], double k2_v[][2-0], double k1_a[][2-0], double buf_p2[][2-0], double buf_p3[][2-0]){
	k2_v[p0][0]	= vxyz[p0][0] + k1_a[p0][0] * dt/2;
	k2_v[p0][1]     = vxyz[p0][1] + k1_a[p0][1] * dt/2;
//	k2_v[p0][2]     = vxyz[p0][2] + k1_a[p0][2] * dt/2;

	double kk2_r12		= pow(pow(buf_p2[p0][0]-buf_p2[p1][0],2) + pow(buf_p2[p0][1]-buf_p2[p1][1],2), 0.5);
...
void rk4_k3(int p0, int p1, const double mass[], double pxyz[][2-0], double vxyz[][2-0], double k3_a[][2-0], double k3_v[][2-0], double k2_a[][2-0], double buf_p3[][2-0], double buf_p4[][2-0]){
	k3_v[p0][0]     = vxyz[p0][0] + k2_a[p0][0] * dt/2;
	k3_v[p0][1]     = vxyz[p0][1] + k2_a[p0][1] * dt/2;
//	k3_v[p0][2]     = vxyz[p0][2] + k2_a[p0][2] * dt/2;

	double kk3_r12         = pow(pow(buf_p3[p0][0]-buf_p3[p1][0],2) + pow(buf_p3[p0][1]-buf_p3[p1][1],2), 0.5);
...
void rk4_k4(int p0, int p1, const double mass[], double pxyz[][2-0], double vxyz[][2-0], double k4_a[][2-0], double k4_v[][2-0], double k3_a[][2-0], double buf_p4[][2-0]){
        k4_v[p0][0]     = vxyz[p0][0] + k3_a[p0][0] * dt;
        k4_v[p0][1]     = vxyz[p0][1] + k3_a[p0][1] * dt;
//        k4_v[p0][2]     = vxyz[p0][2] + k3_a[p0][2] * dt;

        double kk4_r12         = pow(pow(buf_p4[p0][0]-buf_p4[p1][0],2) + pow(buf_p4[p0][1]-buf_p4[p1][1],2), 0.5);
...
void rk4_anchor(int p0, double pxyz[][2-0], double vxyz[][2-0], double k4_a[][2-0], double k3_a[][2-0], double k2_a[][2-0], double k1_a[][2-0], double k4_v[][2-0],  double k3_v[][2-0], double k2_v[][2-0], double k1_v[][2-0]){
...
//	const double Mass[N]= {2.0, 1.0, 0.5};
	const double Mass[N]= {2.0, 1.0};
	double Position[N][2-0], Velocity[N][2-0];
	double K1_a[N][2-0], K2_a[N][2-0], K3_a[N][2-0], K4_a[N][2-0];
	double K1_v[N][2-0], K2_v[N][2-0], K3_v[N][2-0], K4_v[N][2-0];	
	double Buf_p1[N][2-0], Buf_p2[N][2-0], Buf_p3[N][2-0], Buf_p4[N][2-0];
	...
	Velocity[1][1] = 1.0;
        ...
        double r12      = pow(pow(Position[0][0]-Position[1][0],2) + pow(Position[0][1]-Position[1][1],2), 0.5);
        double E01      = ( pow(Velocity[0][0],2) + pow(Velocity[0][1],2) ) * Mass[0]/2 - G*mu1/r12 * mu/Mass[0];
        double E02      = ( pow(Velocity[1][0],2) + pow(Velocity[1][1],2) ) * Mass[1]/2 - G*mu1/r12 * mu/Mass[1];
	...
	for (int st=0; st<100000; st++){
		...
		double rr12      = pow(pow(Position[0][0]-Position[1][0],2) + pow(Position[0][1]-Position[1][1],2), 0.5);
		double E 	= ( pow(Velocity[0][0],2) + pow(Velocity[0][1],2) ) * Mass[0]/2 + ( pow(Velocity[1][0],2) + pow(Velocity[1][1],2) ) * Mass[1]/2 - G*mu1/rr12;
		double err = (E-E01-E02)/(E01+E02);
		cout << "err = " << err << endl;
